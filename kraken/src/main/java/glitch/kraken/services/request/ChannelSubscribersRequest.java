...
public class ChannelSubscribersRequest extends AbstractRestService.AbstractRequest<Subscribers, Subscriber> {
    ...
    public ChannelSubscribersRequest(HttpClient httpClient, HttpRequest<Subscribers> request, Credential credential) {
    ...
    public Mono<Subscribers> get() {
        return Mono.just(checkRequiredScope(credential.getScopes(), GlitchScope.CHANNEL_SUBSCRIPTIONS))
                .flatMap(b -> {
                    if (b) {
                        return exchange().toMono();
                    } else {
                        return Mono.error(handleScopeMissing(GlitchScope.CHANNEL_SUBSCRIPTIONS));
                    }
                });
    }
